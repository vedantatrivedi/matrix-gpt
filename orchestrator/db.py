...
def init_db() -> None:
    ...
    cur.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_recon_battle
        ON recon_results(battle_id, timestamp);
        """
    )
    conn.commit()
    conn.close()
...
def store_recon_result(
    ...
    cur.execute(
        ...
        (id, battle_id, timestamp, endpoint, method, status_code, response_preview, headers, indicators, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            recon_id,
...
def get_recon_results(
    ...
    """Query stored recon results with optional filters."""
    conn = get_conn()
    cur = conn.cursor()

    query = "SELECT * FROM recon_results WHERE battle_id = ?"
    params = [battle_id]
    ...
    for r in rows:
        ...
        try:
            ...
            item["indicators"] = json.loads(item.get("indicators") or "{}")
        except json.JSONDecodeError:
            item["headers"] = {}
            item["indicators"] = {}
        results.append(item)
...
def clear_recon_results(battle_id: str) -> None:
    """Clear all recon results for a battle (useful for re-scanning)."""
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("DELETE FROM recon_results WHERE battle_id = ?", (battle_id,))
    conn.commit()
    conn.close()
